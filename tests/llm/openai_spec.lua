local openai = require("commit.llm.openai")
local parse = openai._parse_response

-- Helper: wrap LLM text output inside a full OpenAI API response envelope.
...
describe("openai._parse_response", function()
  ...
  it("returns error when outer OpenAI response JSON is malformed", function()
  ...
  it("returns API error message when choices is absent", function()
    ...
    assert.is_nil(result)
    assert.equals("Invalid API key", err)
  end)

  it("returns generic error when choices is absent and no error field", function()
