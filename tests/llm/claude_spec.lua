local claude = require("commit.llm.claude")
local parse = claude._parse_response

-- Helper: wrap LLM text output inside a full Anthropic API response envelope.
...
describe("claude._parse_response", function()
  ...
  it("returns error when outer Anthropic response JSON is malformed", function()
  ...
  it("returns API error message when content is absent", function()
    ...
    assert.is_nil(result)
    assert.equals("API rate limit exceeded", err)
  end)

  it("returns generic error when content is absent and no error field", function()
